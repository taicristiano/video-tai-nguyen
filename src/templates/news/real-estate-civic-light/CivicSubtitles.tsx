...
import { COLORS, FONT_BODY, LAYOUT, TYPOGRAPHY } from './tokens';
...
export interface CivicSubtitlesProps {
  slug: string;
  sentenceGap?: number;
  maxWords?: number;
}
...
export const CivicSubtitles: React.FC<CivicSubtitlesProps> = ({
  slug,
  sentenceGap = 0.42,
  maxWords = 10,
}) => {
  ...
  const y = interpolate(localFrame, [0, 14], [8, 0], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
      ...
      {chunk.words.map((word, index) => {
        ...
        return (
          <span
            key={`${chunkIdx}-${index}`}
            style={{
              color: isActive ? COLORS.subtitleActive : isPast ? COLORS.subtitlePast : COLORS.subtitle,
