...
export interface GuideSubtitlesProps {
  slug: string;
  sentenceGap?: number;
  maxWords?: number;
}
...
export const GuideSubtitles: React.FC<GuideSubtitlesProps> = ({
  slug,
  sentenceGap = 0.42,
  maxWords = 10,
}) => {
    ...
    >
      {chunk.words.map((word, index) => (
        <span
          key={`${chunkIdx}-${index}`}
          style={{
            color: index === highlightIdx ? COLORS.blue : index < highlightIdx ? COLORS.faint : COLORS.muted,
            marginRight: 9,
