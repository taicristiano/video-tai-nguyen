...
export interface CampusSubtitlesProps {
  slug: string;
  sentenceGap?: number;
  maxWords?: number;
}
...
const chunkSentence = (sentence: Sentence, maxWords: number): Sentence[] => {
  ...
  for (let i = 0; i < sentence.words.length; i += maxWords) {
    ...
    chunks.push({ words, start: words[0].start, end: words[words.length - 1].end });
  }
  return chunks;
};

export const CampusSubtitles: React.FC<CampusSubtitlesProps> = ({
  slug,
  sentenceGap = 0.42,
  maxWords = 11,
}) => {
  ...
  useEffect(() => {
    fetch(staticFile(`${slug}/timeline.json`))
      ...
      .then((data: Timeline | WordTimestamp[]) => setWords(Array.isArray(data) ? data : data.words ?? []))
      .catch(() => setWords([]));
  }, [slug]);
      ...
      {chunk.words.map((word, index) => {
        ...
        return (
          <span
            key={`${chunkIdx}-${index}`}
            style={{
              color: isActive ? COLORS.coral : isPast ? COLORS.indigo : COLORS.muted,
              marginRight: 9,
