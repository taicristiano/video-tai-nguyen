...
export interface EntertainmentSubtitlesProps {
  slug: string;
  sentenceGap?: number;
  maxWords?: number;
}
...
export const EntertainmentSubtitles: React.FC<EntertainmentSubtitlesProps> = ({
  slug,
  sentenceGap = 0.42,
  maxWords = 10,
}) => {
      ...
      {chunk.words.map((word, index) => {
        ...
        return (
          <span
            key={`${chunkIdx}-${index}`}
            style={{
              color: isActive ? COLORS.coralDark : isPast ? COLORS.plum : COLORS.muted,
              marginRight: 9,
