...
import { COLORS, FONT_SANS, LAYOUT, TYPOGRAPHY } from './tokens';
...
export interface TravelSubtitlesProps {
  slug: string;
  sentenceGap?: number;
  maxWords?: number;
}
...
export const TravelSubtitles: React.FC<TravelSubtitlesProps> = ({
  slug,
  sentenceGap = 0.42,
  maxWords = 10,
}) => {
      ...
      {chunk.words.map((word, index) => {
        ...
        return (
          <span
            key={`${chunkIdx}-${index}`}
            style={{
              color: isActive ? COLORS.coral : isPast ? COLORS.faint : COLORS.muted,
              marginRight: 9,
