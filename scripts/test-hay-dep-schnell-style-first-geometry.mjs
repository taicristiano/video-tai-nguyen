...
const BASE_DIR = path.join(ROOT, 'scratch', 'v33', 'schnell-style-first');
...
export const BEAT_SPECS = {
  'beat-01': {
    role: 'establish',
    cast: 'CAST:\nfather (sage shirt), mother (cardigan), boy (sage tee), girl (cream dress).',
  ...
  'beat-04': {
    role: 'interaction',
    cast: 'CAST:\nboy (sage tee), father (sage shirt), mother (cardigan).',
...
export function buildStyleFirstPrompt(beatId) {
...
async function main() {
  ...
  console.log('=== HAY & ĐẸP. V3.3B-S.1: Schnell Style-First Prompt Geometry Test ===');
  ...
  for (const beatId of TARGET_BEATS) {
    ...
    const prompt = buildStyleFirstPrompt(beatId);
    ...
    console.log(`[${beatId}] (${beat.storyRole}) Length: ${prompt.length} chars (Target <= 1400, Max 1600: ${prompt.length <= 1600 ? 'PASS' : 'FAIL'})`);
    if (prompt.length > 1600) {
      throw new Error(`Beat ${beatId} prompt exceeded 1600 characters (${prompt.length})!`);
    }
  }
  ...
  if (args.generate || args.contactSheet) {
    ...
    for (const beatId of TARGET_BEATS) {
      ...
      for (const variant of ['a', 'b']) {
        ...
        sheetImages.push({
          path: outPath,
          label: `${beatId} ${variant.toUpperCase()} • ${beat.storyRole.toUpperCase()}`,
    ...
    await createGridContactSheet({
      ...
      title: 'HAY & ĐẸP. V3.3B-S.1 — Schnell Style-First Prompt Geometry Test',
